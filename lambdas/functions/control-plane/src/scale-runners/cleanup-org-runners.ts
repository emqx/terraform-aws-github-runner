...
async function listGitHubRunners(ghClient: Octokit, runnerOwner: string, runnerType: string): Promise<GhRunners> {
...
async function deleteOfflineRunners(ghClient: Octokit, runnerOwner: string, runnerLabels: string[]): Promise<void> {
  const ghRunners = await listGitHubRunners(ghClient, runnerOwner, 'Org');
  await Promise.all(
